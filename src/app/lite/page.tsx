...
export default function LiteWizardPage() {
  ...
  // Step 4 State: Essay Writer & Auto-Generate
  ...
  const handleSelectIdea = async (idea: DetailedEssayIdea) => {
    ...
    // Auto-generate full essay draft incorporating Story Vault if editor is empty
    if (!essayText.trim()) {
      setAutoDraftLoading(true);
      try {
        const draft = await generateFullEssayDraft(selectedPrompt, profile, vaultStories, idea.title);
        setEssayText(draft);
        localStorage.setItem("essayforge_lite_essay", draft);
      } catch (e) {
        // Fallback title setup
  ...
  // Explicit Auto-Generate Draft Button Action
  const handleAutoGenerateDraft = async () => {
    setAutoDraftLoading(true);
    try {
      const draft = await generateFullEssayDraft(selectedPrompt, profile, vaultStories, selectedIdeaTitle);
      setEssayText(draft);
      localStorage.setItem("essayforge_lite_essay", draft);
    } catch (e) {
      // Fallback
            ...
            <CardContent className="p-6 space-y-3">
              {autoDraftLoading ? (
                <div className="py-20 text-center space-y-3 bg-secondary/30 rounded-2xl border border-dashed border-orange-500/30">
                  ...
                  <p className="text-sm font-bold text-foreground">Drafting full Common App essay using your Story Vault...</p>
                  <p className="text-xs text-muted-foreground">Incorporating anecdotes, background details, and reflective turning points.</p>
                </div>
              ) : (
                <>
                  <Textarea
