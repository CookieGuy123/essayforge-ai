...
export default function LiteWizardPage() {
  ...
  // Step 4 State: Essay Writer & Dual Draft Options (With vs Without Stories)
  ...
  const handleSelectIdea = async (idea: DetailedEssayIdea) => {
    ...
    // Auto-generate full essay draft if editor is empty
    if (!essayText.trim()) {
      setAutoDraftLoading(true);
      try {
        const draft = await generateFullEssayDraft(selectedPrompt, profile, vaultStories, idea.title, useStoriesOption);
        setEssayText(draft);
  ...
  // Explicit Auto-Generate Draft Action (With vs Without Stories)
  const handleAutoGenerateDraft = async (withStories: boolean = useStoriesOption) => {
    setAutoDraftLoading(true);
      ...
      <div className="w-full max-w-4xl flex-1 flex flex-col">
        ...
        {currentStep === 4 && (
          <Card className="border-border/40 shadow-sm animate-card-pop">
            ...
            <CardContent className="p-6 space-y-4">
              {/* Draft Style Selector (With Stories vs Without Stories) */}
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between p-2 rounded-2xl bg-secondary/50 border border-border/40 gap-2">
                <span className="text-xs font-bold text-muted-foreground px-2">Draft Generation Style:</span>
                <div className="flex items-center gap-2">
                  <button
                    disabled={autoDraftLoading}
                    onClick={() => handleAutoGenerateDraft(true)}
                  ...
                  <button
                    disabled={autoDraftLoading}
                    onClick={() => handleAutoGenerateDraft(false)}
                    className={`flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold transition-all ${
                      !useStoriesOption
                        ? "bg-gradient-to-r from-orange-500 to-amber-600 text-white shadow-sm"
                        : "text-muted-foreground hover:text-foreground hover:bg-card"
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleAutoGenerateDraft(useStoriesOption)}
