...
export function Sidebar() {
  ...
  return (
    <aside className="w-64 border-r border-border/40 bg-card dark:bg-slate-950 flex flex-col justify-between hidden md:flex shrink-0 min-h-screen transition-colors">
      <div>
        ...
        >
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-2xl bg-gradient-to-tr from-orange-500 via-amber-500 to-red-600 flex items-center justify-center text-white shadow-md shadow-orange-500/25 shrink-0 group-hover:scale-105 group-hover:rotate-6 transition-all duration-300">
              <Flame className="h-6 w-6" />
            </div>
            <div>
              <h1 className="font-extrabold text-xl leading-tight tracking-tight text-orange-600 dark:text-orange-400 group-hover:text-orange-500 transition-colors">
                EssayForge Pro
              </h1>
            ...
            return (
              <Link
                ...
                className={`group flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm transition-all duration-200 ease-out ${
                  isActive
                    ? "bg-gradient-to-r from-orange-500 via-amber-500 to-red-600 text-white font-bold shadow-md shadow-orange-500/30 translate-x-1 scale-[1.01]"
                    : item.highlight
                    ? "text-orange-600 dark:text-orange-400 font-medium hover:bg-orange-500/15 hover:translate-x-1.5 hover:text-orange-700 dark:hover:text-orange-300"
                    : "text-muted-foreground font-medium hover:text-foreground hover:bg-secondary/80 hover:translate-x-1.5"
                }`}
              >
                <Icon className={`h-4 w-4 shrink-0 transition-transform duration-200 group-hover:scale-110 ${
                  isActive ? "text-white" : item.highlight ? "text-orange-500 dark:text-orange-400" : "text-muted-foreground group-hover:text-foreground"
                }`} />
                <span className="truncate">{item.name}</span>
                {item.highlight && !isActive && (
                  <span className="ml-auto text-[10px] uppercase font-extrabold px-1.5 py-0.5 rounded-md bg-orange-100 dark:bg-orange-950/80 border border-orange-500/30 text-orange-700 dark:text-orange-300 group-hover:bg-orange-200 dark:group-hover:bg-orange-900 transition-colors">
                    Core
                  </span>
                )}
              </Link>
      ...
      <div className="p-4 m-4 rounded-2xl bg-gradient-to-b from-orange-500/10 to-red-500/10 border border-orange-500/20 text-xs text-muted-foreground space-y-2 hover:border-orange-500/40 transition-all duration-300">
